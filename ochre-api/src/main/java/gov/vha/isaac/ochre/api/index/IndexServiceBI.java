...
/**
 * The contract interface for indexing services. <br>
 * {@code IndexService} implementations must not throw exceptions. Throwing
 * exceptions could cause the underlying source data to corrupt. Since indexes
 * can be regenerated, indexes should mark themselves as invalid somehow, and
 * recreate themselves when necessary.
 * 
 * @author aimeefurber
 * @author kec
 */
...
public interface IndexServiceBI extends DatabaseServices {
	...
	/**
	 * Clear index, resulting in an empty index. Used prior to the environment
	 * recreating the index by iterating over all components and calling the
	 * {@code index(ComponentChronicleBI chronicle)} with each component of the
	 * iteration. May be used for initial index creation, or if indexing
	 ...
	 * forceMerge is a costly operation, so generally call it when the index is
	 * relatively static (after finishing a bulk addition of documents)
	 */
	void forceMerge();
	 ...
	 *
	 * @param nid
	 *            for the component that the caller wished to wait until it's
	 *            document is added to the index.
	 * @return a {@code Callable&lt;Long&gt;} object that will block until this
	 *         indexer has added the document to the index. The {@code call()}
	 *         method on the object will return the index generation that
	 *         contains the document, which can be used in search calls to make
	 *         sure the generation is available to the searcher.
	 */
	...
	/**
	 * Index the chronicle in a manner appropriate to the indexer
	 * implementation. The implementation is responsible to determine if the
	 * component is appropriate for indexing. All changed components will be
	 * sent to all indexers for indexing. The implementation must not perform
	 * lengthy operations on this thread.
	 *
	 * @param chronicle
	 * @return a {@code Future<Long>}for the index generation to which this
	 *         chronicle is attached. If this chronicle is not indexed by this
	 *         indexer, the Future returns {@code Long.MIN_VALUE
	 *         {@code . The generation can be used with searchers
	...
	/**
	 * @return name / value pairs that give statistics on the number of things
	 *         indexed since the last time #clearIndexedStatistics was called.
	 */
	HashMap<String, Integer> reportIndexedItems();

	/**
	 * Zero out the statistics that would be reported by
	 * {@link #reportIndexedItems()}
	 */
	void clearIndexedStatistics();

	/**
	 * Enables or disables an indexer. A disabled indexer will take no action
	 * when the index method is called.
	 * 
	 * @param enabled
	 *            true if the indexer is enabled, otherwise false.
	 */
	 ...
	 *            (optional) target generation that must be included in the
	 *            search or Long.MIN_VALUE if there is no need to wait for a
	 *            target generation. Long.MAX_VALUE can be passed in to force
	 ...
	 * together, maintaining the best score. This is a convenience method.
	 * 
	 * @param searchResult
	 * @return the merged results, in a collection that iterates in the same
