...
public class UuidFactory {
    ...
    public static UUID getUuidForLogicGraphSememe(UUID authority, UUID assemblage, UUID refComp, Object[] parameters) {
        ...
        return UuidT5Generator.get(authority, createUuidTextSeed(assemblage.toString(), refComp.toString(), logicGraphStr));
    }

    public static UUID getUuidForMemberSememe(UUID authority, UUID assemblage, UUID refComp) {
        return UuidT5Generator.get(authority, createUuidTextSeed(assemblage.toString(), refComp.toString(), MEMBER_SEED_STRING));
    }
    ...
    public static UUID getUuidForDynamicSememe(UUID authority, UUID assemblage, UUID refComp,  Object[] dynamicData) {
        if (dynamicData != null && dynamicData.length > 0) {
            DynamicSememeData[] dataArray = ((AtomicReference<DynamicSememeData[]>) dynamicData[0])
                    .get();
            ...
            return UuidT5Generator.get(authority, createUuidTextSeed(assemblage.toString(), refComp.toString(), EMPTY_CONTENT_DYNAMIC_SEED_STRING));
        }
    }


    public static UUID getUuidForComponentNidSememe(UUID authority, UUID assemblage, UUID refComp, UUID component) {
        return UuidT5Generator.get(authority, createUuidTextSeed(assemblage.toString(), refComp.toString(), component.toString()));
    }
    ...
    public static UUID getUuidForDescriptionSememe(UUID authority, int assemblageSeq, UUID concept, int caseSigNid,
        ...
        return UuidT5Generator.get(UuidT5Generator.PATH_ID_FROM_FS_DESC,
                createUuidTextSeedForDescription(Get.identifierService().getUuidPrimordialForNid(assemblageNid).get(), concept, 
                    Get.identifierService().getUuidPrimordialForNid(caseSigNid).get(),
                        descType, language, descriptionText));
    }

    public static UUID getUuidForStringSememe(UUID authority, UUID assemblage, UUID refComp, Object[] parameters) {
        return UuidT5Generator.get(authority, createUuidTextSeed(assemblage.toString(), refComp.toString(), (String)parameters[0]));
    }
    ...
    public static UUID getUuidForDescriptionSememe(UUID authority, UUID assemblage, UUID concept, Object[] parameters) {
        ...
        return UuidT5Generator.get(authority,
                createUuidTextSeedForDescription(assemblage, concept,
                        ...
                        Get.identifierService().getUuidPrimordialForNid(languageNid).get(),
                        (String) parameters[3]));
    }

    private static String createUuidTextSeedForDescription(UUID assemblage, UUID concept, UUID caseSignificance, UUID descriptionType, UUID language,
            String descriptionText) {
        return createUuidTextSeed(assemblage.toString(), concept.toString(), caseSignificance.toString(), descriptionType.toString(), language.toString(), descriptionText);
    }
