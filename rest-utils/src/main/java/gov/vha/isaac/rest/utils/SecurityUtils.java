	...
	public static void validateRole(SecurityContext securityContext, Method method) {
		...
		if (denyAll != null) {
			throw new SecurityException("User not authorized: " 
					+ ((securityContext.getUserPrincipal() == null ? "-unknown user-" : securityContext.getUserPrincipal().getName())) + " to access methods of class " 
			...
			if (annotation.annotationType() == RolesAllowed.class
				...
				if (existingRoleBasedSecurityAnnotation == null) {
					...
					throw new RuntimeException("Cannot apply both annotation-based role-related security constraints to class " + apiClass.getSimpleName() 
						+ ": " + annotation.annotationType().getSimpleName() + " and " + existingRoleBasedSecurityAnnotation.annotationType().getSimpleName());
				}
			}
		}
	}
