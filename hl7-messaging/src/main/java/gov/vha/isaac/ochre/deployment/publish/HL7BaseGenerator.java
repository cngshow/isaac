...
public class HL7BaseGenerator
{
	...
	/**
	 * The <code>getMessage</code> method converts from a MFN_M01 message
	...
	/**
	 * Converts the boolean status value retrieved from the database
	 * and converts it to a value recognized by VistA
	 * @param conceptStatus
	 * @return String
	 */
	...
	/**
	 * The <code>getCombinedMessage</code> method returns a single message in the form
	 * of a String. This single message represents a concatenation of all
	 * messages in the List that was passed to the method. The concatenation is
	 * done in List order. Resulting message has a single HL7 message header.
	 *
	 * @param messages
	...
	public static String getCombinedMessage(List<String> messages) throws STSException
	{
		...
		//remove any messages that are zero-length or null and add to new List<String>
		List<String> cleanedMessageList = new ArrayList<>();
		...
		try
		{
			//TODO: Verify this is correct.  The methods below to set date/time are not longer valid in newer Hapi libraries.
			...
			Calendar date;
			try {
				date = parseDate(hl7DateString);
				MfnM01Message.getMSH().getDateTimeOfMessage().setValueToMinute(date);
				MfnM01Message.getMFI().getEffectiveDateTime().setValueToMinute(date);
				MfnM01Message.getMFI().getEnteredDateTime().setValueToMinute(date);
	...
	private static Calendar parseDate(String inDate) throws ParseException
	{
