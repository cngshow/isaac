 ...
 *
 * An implementation of a MetaContentService wrapped around the MVStore from the H2 DB project
...
public class MVStoreMetaContentProvider implements MetaContentService
{
	...
	private MVStoreMetaContentProvider()
	{
		...
		if (!temp.isDirectory())
		{
			throw new RuntimeException("Cannot initialize MetaContent Store - was unable to create " + temp.getAbsolutePath());
		}
		initialize(temp, "service_", false);
	}
	
	/**
	 * Typically, this object should be retrieved from HK2 / the Lookup service - which already had a Service instance created.
	 * However, it is allowable to create your own instance outside of the management of HK2 using this method.
	 * @param storageFolder - The folder to utilize for storage.
	 * @param storePrefix - optional - a prefix to utilize on all files/folders created by the service inside the storageFolder
	 * @param wipeExisting - true to erase preexisting content and start fresh, false to read existing data.
	 */
	...
	/**
	 * @see gov.vha.isaac.ochre.api.metacontent.MetaContentService#putUserPrefs(int, gov.vha.isaac.ochre.api.metacontent.userPrefs.StorableUserPreferences)
	 */
	@Override
	public byte[] putUserPrefs(int userId, StorableUserPreferences userPrefs)
	{
		return userPrefsMap.put(userId > 0 ? userId : Get.identifierService().getConceptSequence(userId), userPrefs.serialize());
	}
