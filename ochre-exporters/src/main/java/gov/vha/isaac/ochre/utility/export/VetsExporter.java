...
public class VetsExporter {
	...
	public void export(OutputStream writeTo, long startDate, long endDate) {
		...
		//from Randall: "TDS will give an error for empty tags."
		//So either comment out here or determine value in String csCopyrightURL above
		...
		//from Randall: "TDS will give an error for empty tags (even those that are optional)"
		...
		Get.conceptService().getConceptChronologyStream().forEach((_concept) -> {
			...
			else
			{
				...
				else
				{
					...
					_xmlCodedConcept.setAction(determineAction(_concept, startDate, endDate));
		...
		Get.conceptService().getConceptChronologyStream().forEach((_concept) -> {
			...
			if (Frills.definesMapping(_concept.getConceptSequence())) {
				...
				_xmlMapSet.setAction(determineAction(_concept, startDate, endDate));
	...
	private Map<String, Object> getProperties(SememeChronology<?> sememe, long startDate, long endDate) {
		...
		if (sememeVersion.isPresent()) {
			...
			{
				action = determineAction(sc, startDate, endDate);
			} else {
